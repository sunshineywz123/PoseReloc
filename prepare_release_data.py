...
# COPY_DIRS = ["color", "intrin_ba", "poses_ba", "reproj_box", 'intrinsics.txt']
...
def parse_args():
    ...
    parser.add_argument(
        "--copy_type", choices=['soft_link', 'copy'], default='copy'
    )
    parser.add_argument(
        "--copy_img_full", action='store_true'
    )
    parser.add_argument(
        "--change_id", action='store_true'
    )

    parser.add_argument(
        "--only_first_two_seq", action='store_true' # For onepose lowtexture: True
    )

    parser.add_argument(
        "--output_path", type=str, default="/cephfs-mvs/3dv-research/hexingyi/nips_data_release_final",
    )
...
if __name__ == '__main__':
    ...
    if args.copy_img_full:
        COPY_DIRS.append('color_full')
    ...
    for idx, obj_dir in tqdm(enumerate(aim_obj_dirs), total=len(aim_obj_dirs)):
        ...
        if output_obj_dir.exists():
            os.system(f"rm -rf {str(output_obj_dir)}")
        ...
        for ply_path in obj_dir.glob('*.ply'):
            ...
            elif args.copy_type == 'soft_link':
                os.system(f"ln -s {str(ply_path)} {str(output_scan_obj_dir / ply_path.name)}")
        for texture_path in obj_dir.glob('*.jpg'):
            ...
            if args.copy_type == 'copy':
                ...
                os.system(f"ln -s {str(texture_path)} {str(output_scan_obj_dir / texture_path.name)}")
        ...
        for sub_seq in sub_seqs:
            ...
            if args.only_first_two_seq and int(seq_id) > 2:
                continue
            ...
            for preserve_file_name in COPY_DIRS:
                ...
                if preserve_file_name == 'intrinsics.txt' and not src_path.exists():
                    intrin_dir = sub_seq / "origin_intrin"
                    intrin_paths = intrin_dir.glob("*.txt")
                    intrin = [get_K(str(intrin_path))[0] for intrin_path in intrin_paths]
                    intrin_avg = np.average(np.stack(intrin), axis=0)
                    np.savetxt(str(output_seq_dir / src_path.name), intrin_avg)
                else:
                    if not src_path.exists():
                        logger.warning(f"{str(src_path)} not exists!")
                    if args.copy_type == 'copy':
                        os.system(f"cp -r {str(src_path)} {str(output_seq_dir)}")
                    elif args.copy_type == 'soft_link':
                        os.system(f"ln -s {str(src_path)} {str(output_seq_dir / src_path.name)}")
