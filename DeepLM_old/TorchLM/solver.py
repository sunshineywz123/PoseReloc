...
class StepEvaluator:
    ...
    def accept(self, cost, modelCostChange):
        ...
        if self.numConsecutiveNonmonotonicSteps == self.maxConsecutiveNonmonotonicSteps:
...
class LMSolver:
    ...
    def InitializeVariables(self):
        ...
        for v in self.variables:
            ...
            for j in range(1, len(v.shape)):
                l *= v.shape[j]
            self.preconditioner.append(
                torch.zeros((v.shape[0], l, l), dtype=v.dtype, device=v.device)
            )
    ...
    def EvaluateCost(self, candidate):
        ...
        for funcId in range(len(self.functions)):
            ...
            for dim in range(0, residualNum, span):
                ...
                varIndexed = [
                    variables[i][indices[i][start:end]] for i in range(len(variables))
                ]
                constantsPar = [constants[i][start:end] for i in range(len(constants))]

                residuals = func.fn(*varIndexed, *constantsPar, marker=False)
    ...
    def Evaluate(self, isFirstTime=False):
        ...
        for funcId in range(len(self.functions)):
            ...
            if isFirstTime:
                ...
                constantTemp = [constants[i][:1] for i in range(len(constants))]
                residualsTemp = fn(*varTemp, *constantTemp, marker=False) # NOTE: append_maker para should be removed, and is only useful in second order optimizer visualization
                ...
                self.functions[funcId].buffer = torch.zeros(
                    indices[0].shape[0],
                    maxDim,
                ...
                self.functions[funcId].residuals = torch.zeros(
                    residualNum,
                    residualDim,
                    device=indices[0].device,
                    dtype=variables[0].dtype,
            ...
            for dim in range(0, residualNum, span):
                ...
                residuals = fn(*varIndexed, *constantsPar, marker=True)
                ...
                for i in range(residuals.shape[1]):
                    ...
                    for j in vrange:
                        ...
                        if grad is None:
                            self.functions[funcId].jacobians[j][i][start:end].zero_()
                            continue
                        grad = grad.data
                        grad = grad.view(grad.shape[0], grad.shape[1], -1)
                        self.functions[funcId].jacobians[j][i][start:end].copy_(grad)

                    del l

                self.functions[funcId].residuals[start:end].copy_(residuals.data)
                self.cost += cost.item()
            ...
            # 		gradients)
            # else:
            LMCore.JacobiLeftMultiply(
                jacobians,
                residuals,
                indices,
                self.functions[funcId].buffer,
                gradients,
                0,
            )
            ...
            jacobianScale = [self.jacobianScale[i] for i in self.vidsFunc[funcId]]
    ...
    def JacobiJtJD(self, diagonal, p, residuals, jtrs):
        ...
        for i in range(len(self.functions)):
            ...
            LMCore.JacobiLeftMultiply(
                jacobians, residuals[i], indices, buf, jtrsTemp, 0
            )
    ...
    def LinearSolve(self, lmDiagonal):
        ...
        for i in range(len(self.functions)):
            ...
            LMCore.JacobiLeftMultiply(jacobians, residuals, indices, buf, brefTemp, 0)
        ...
        for i in range(len(self.functions)):
            ...
            LMCore.JacobiBlockJtJ(jacobians, lmDiagonalTemp, indices, preconditioner, 0)
        ...
        while True:
            ...
            if self.summary.numIterations % self.option.residualResetPeriod == 0:
                # if self.isCuda:
    ...
    def ComputeTrustRegionStep(self):
        ...
        for i in range(len(self.functions)):
            ...
            LMCore.JacobiRightMultiply(jacobians, stepTemp, indices, modelResiduals[i])

        self.modelCostChange = 0

        for i in range(len(self.functions)):
            self.modelCostChange += -torch.sum(
                modelResiduals[i]
        ...
        if self.summary.stepIsValid:
            self.delta = [step[i] * self.jacobianScale[i] for i in range(len(step))]
    ...
    def Solve(self):
        ...
        if self.verbose:
            print(
                "Initial cost = %E, Memory = %E G"
                % (self.cost, self.Memory() / 1024.0 / 1024.0 / 1024.0)
            )
        initial_cost = self.cost
        finial_cost = self.cost

        self.modelResiduals = [
            self.functions[funcId].residuals.clone()
            for funcId in range(len(self.functions))
        ]
        ...
        successIterations = 0
        self.debug = False
        current_iter_start_time = self.Timing()
        while True:
            t1 = self.Timing()
            outerIterations += 1
            if outerIterations == self.option.maxNumIterations:
                print("exit by:if outerIterations == self.option.maxNumIterations Ln:645") if self.verbose else None
                break
            ...
            if not self.summary.stepIsValid:
                ...
                if self.summary.numConsecutiveInvalidSteps > self.option.maxInvalidStep:
                    self.summary.lmTerminationType = 2
                    print("exit by:if self.summary.numConsecutiveInvalidSteps > self.option.maxInvalidStep Ln:652") if self.verbose else None
                    break
                self.strategy.reject()
                continue

            candidateX = [
                self.variables[i].data
                + self.option.stepScale * self.delta[i].reshape(self.variables[i].shape)
                for i in self.vranges
            ]
            ...
            if stepNorm < stepSizeTolerance:
                ...
                print("exit by:if stepNorm < stepSizeTolerance Ln:674") if self.verbose else None
                break

            # function tolerance check
            costChange = self.cost - cost
            absoluteFunctionTolerance = self.option.functionTolerance * self.cost

            if abs(costChange) < absoluteFunctionTolerance:
                self.summary.lmTerminationType = 1
                print("exit by:if abs(costChange) < absoluteFunctionTolerance Ln: 683") if self.verbose else None
                break
            ...
            # FIXME: Problem may be here! convex function assumption?
            if self.summary.relativeDecrease > self.option.minRelativeDecrease:
                for i in self.vranges:
                    self.variables[i].data += self.option.stepScale * self.delta[
                        i
                    ].reshape(self.variables[i].shape) # update by hand!
                ...
                if self.verbose:
                    ...
                    currentTime = time()
                    print(
                        "iter = %d, residual = %E, radius = %E, numLinearIterations = %d, current_iter_time = %f ms ,total_time = %f ms"
                        % (
                            outerIterations,
                            cost,
                            self.strategy.radius,
                            self.summary.numIterations,
                            (currentTime - current_iter_start_time) * 1000,
                ...
                if successIterations >= self.option.maxSuccessIterations:
                    self.cost = cost
                    print('exit by:if successIterations >= self.option.maxSuccessIterations Ln:722') if self.verbose else None
                    break
            ...
            if self.strategy.radius < 1e-32 or self.summary.gradientMaxNorm < 1e-10:
                self.summary.lmTerminationType = 1
                print('exit by: if self.strategy.radius < 1e-32 or self.summary.gradientMaxNorm < 1e-10 Ln:734') if self.verbose else None
                break
        ...
        if self.verbose:
            print(
                "LM optimizer initial cost = %E , finial cost = %E, decrease = %E, relative decrease percent = %f%%"
                % (initial_cost, finial_cost, initial_cost - finial_cost, ((initial_cost - finial_cost)/(initial_cost + 1e-4))*100)
            )
...
def Solve(
    ...
    fn,
    numIterations=150,
    numSuccessIterations=150,
    maxLinearIterations=150,  # above three only config part of optimization paras and rest of paras default
    optimization_cfgs=None,  # config for all optimization pars, input configs will over write default configs
    verbose=True,
    **kargs
):
    # device check: all variables and constants should be located at same device
    device = variables[0].device
    for item in [*variables, *constants]:
        assert item.device == device, "device in-consistency! please check"
    if len(indices) == 0:
        return
    ...
    func = FunctionBlock(
        variables=variables, constants=constants, indices=indices, fn=fn
    )

    if optimization_cfgs is None:
        option = LMOption()
        option.maxLinearIterations = maxLinearIterations
        option.maxNumIterations = numIterations
        option.maxSuccessIterations = numSuccessIterations
    else:
        option = LMOption(optimization_cfgs)
