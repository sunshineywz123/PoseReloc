...
def nn(cfg, feature_path, covis_pairs, matches_out, vis_match=False):
    """Match features by nearest neighbour"""
    ...
    model = NearestNeighbour(conf).cuda()
...
def spg(cfg, feature_path, covis_pairs, matches_out, vis_match=False):
    ...
    model.eval()
    load_network(model, cfg.network.matching_model_path)
