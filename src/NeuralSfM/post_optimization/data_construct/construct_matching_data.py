...
class MatchingPairData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        valid_kpts_idxs = np.arange(left_frame_info['keypoints'].shape[0])[valid_kpts_mask]
        ...
        for i, related_3d_id in enumerate(related_3d_ids.tolist()):
            ...
            if len(related_index) != 0:
                ...
                point2d_idx = self.colmap_3ds[related_3d_id].point2D_idxs[np.squeeze(related_index).tolist()]  # int
                left_kpts.append(valid_kpts[i]) # FIXME: duplicate points in a feature track is counted twice!
                ...
                self_img_id, self_kpt_idx = self.colmap_image_dataset.point_cloud_assigned_imgID_kptID[related_3d_id]
