...
def search_raw_array_pytorch(res, xb, xq, k, D=None, I=None, metric=faiss.METRIC_INNER_PRODUCT):
...
def knn_faiss_raw(feat0, feat1, k):
    ...
    # TODO: consider mask
    if b == 1:
        # fmap1 = feat0.view(ch, -1).t().contiguous()
        # fmap2 = feat1.view(ch, -1).t().contiguous()
        feat0, feat1 = feat0.view(-1, ch).contiguous(), feat1.view(-1, ch).contiguous()

        dist, indx = search_raw_array_pytorch(res, feat1, feat0, k, metric=faiss.METRIC_INNER_PRODUCT)
