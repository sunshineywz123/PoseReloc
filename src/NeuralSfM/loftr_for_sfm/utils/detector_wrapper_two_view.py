    ...
    def forward(self, batch):
        ...
        Update:
            batch (dict):{
                ...
                "detector_scores0": [M]    (Optional)
                "detector_b_ids0": [M]       (Optional)
            }
        """
        if self.detector_type == 'OnGrid':
            ...
            # update batch
            B, device = batch['image0'].shape[0], batch['image0'].device
            ...
            if not self.read_keypoints:
                ...
                ret_dict = {"keypoints": [batch['keypoints0'][b] for b in range(B)],
                            "scores": [batch['scores0'][b] for b in range(B)]}  # TODO: batch-size == 1 only?
            ...
            batch.update({
                ...
                'detector_b_ids0': bids.long().to(device),  # [M]
            })
    ...
    def forward(self, batch):
        ...
        batch.update({
            **batch0,
            ...
            'detector_b_ids1': batch1['detector_b_ids0'],
