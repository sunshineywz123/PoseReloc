...
----------------------------------------------------------------
Modification: Spatially adaptive kernel selection.

Modified based on the impl of Ross Wightman
"""
import torch
...
def build_ssk_merge(config):
    if config['ssk']['enable']:
        return SpatialSelectiveKernelMerge(config)
    else:
        return PlaceHolder()
...
class PlaceHolder(nn.Module):
    ...
    def forward(self, feat_conv, feat_loftr):
        return feat_loftr
...
class SpatialSelectiveKernelAttn(nn.Module):
    def __init__(self, channels, num_paths=2, attn_channels=32,
                 act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d):
        """ (Spatially Adaptive) Selective Kernel Attention Module
    ...
    def forward(self, x):
        ...
        x = x.sum(1)  # .mean((2, 3), keepdim=True) (N, C, H, W)
        x = self.fc_reduce(x)
        x = self.bn(x)
        x = self.act(x)
        x = self.fc_select(x)
        ...
        x = torch.softmax(x, dim=1)  # (N, 2, C, H, W)
        return x
    

class SpatialSelectiveKernelMerge(nn.Module):
    def __init__(self, cfg, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d):
        super().__init__()
        self.attn = SpatialSelectiveKernelAttn(cfg['d_model'], 
                                               attn_channels=cfg['ssk']['d_attn'],
                                               act_layer=act_layer, norm_layer=norm_layer)
        self.residual = cfg['ssk']['residual']
        
    def forward(self, feat_conv, feat_loftr):
        residual = feat_conv
        feat_pure_loftr = feat_loftr - feat_conv
        x = torch.stack([feat_conv, feat_pure_loftr], dim=1)
        ssk_attn = self.attn(x)
        x = torch.sum(x * ssk_attn, 1)
        x = x + residual if self.residual else x
        return x


class SpatialSelectiveKernelConv(nn.Module):
