...
def patch2pix_merger(matches_dict, name_split=' ', sc_thres=0.25, qt_dthres=4, qt_psize=48, qt_unique=True):
    """
    ...
    for pair_name, matches_info in matches_dict.items():
        name0, name1 = pair_name.split(name_split)

        matches = matches_info[:, :4]
        scores = matches_info[:, 4]
        ...
        # Save matches
        matches_ids_dict[pair_name] = match_ids
        
    # Save keypoints
    for name in all_kp_data:
        kps = np.array(all_kp_data[name]['kps'], dtype=np.float32)
        keypoints_dict[name] = kps
    
    return keypoints_dict, matches_ids_dict
