...
# from src.loftr_sfm import LoFTR_SfM
...
from src.matcher_loftr.utils.detector_wrapper import (
    DetectorWrapper,
...
def parse_args():
    ...
    parser.add_argument(
        "--match_type",
        type=str,
        choices=["loftr_coarse", "loftr_all", "SuperGlue"],
    ...
    # parser.add_argument("--subset_path", default=None, help="Path to subset images")
    parser.add_argument(
        "--dataset_name",
        type=str,
        default="IMC",
    ...
    parser.add_argument("--use_imc_pose_error_method", action="store_true")
    parser.add_argument("--discard_penalty", action="store_true")
    ...
    # ) if args.subset_path is not None else None
...
def build_model(args):
    ...
    if args.match_type == "loftr_coarse" or args.match_type == "loftr_all":
...
@torch.no_grad()
def match_worker(dataset, subset_ids, args):
...
def keypoint_worker(name_kpts, args):
    """merge keypoints associated with one image.
    python >= 3.7 only.
    """
    keypoints = {}
    for name, kpts in tqdm(name_kpts):
        # filtering
...
# @ray.remote(num_cpus=1)
def update_matches(matches, keypoints, args):
...
# @ray.remote(num_cpus=1)
def transform_keypoints(keypoints, args):
...
def exhaustive_matching(args, dataset):
    ...
    if args.cache and osp.exists(osp.join(args.cache_dir, "raw_matches.h5")):
        ...
        logger.info("Matching image pairs....")
        ...
        matches = match_worker(dataset, range(len(dataset)), args)
    ...
    sub_kpts = chunks(all_kpts, math.ceil(n_imgs / 1))
    obj_refs = [keypoint_worker(sub_kpt, args) for sub_kpt in sub_kpts]
    keypoints = dict(ChainMap(*obj_refs))

    # Convert keypoints match to keypoints indexs
    logger.info("Update matches")
    obj_refs = [
        update_matches(sub_matches, keypoints, args)
    ...
    kpts_scores = [
        transform_keypoints(sub_kpts, args)
...
def visualize_colmap(dataset, keypoints_dict, args):
    ...
    if is_colmap_valid:
        ...
        logger.warning(f"COLMAP failed on bag: {dataset.subset_name}!")
        colmap_images = None
    ...
    for i, image_path in enumerate(img_paths):
        ...
        if best_index != -1 and colmap_images is not None:
    ...
    registred_mask = [False for i in range(len(img_paths))]
    for index in registed_index:
        registred_mask[index] = True

    return registred_mask
...
def eval_colmap_results(dataset, args):
    ...
    if is_colmap_valid:
        ...
        result = [
            compute_stereo_metrics_from_colmap(
                ...
                best_index,
                colmap_output_path,
                use_imc_pose_error_method=args.use_imc_pose_error_method,
            )
            for pair in tqdm(pair_ids)
        ]
    ...
    file_name_part.append(
        "_colmap_err"
    ) if args.use_imc_pose_error_method else file_name_part.append("_loftr_err")
    file_name_part.append("_discard_penalty") if args.discard_penalty else None
    file_name_part.append(".txt")
    file_name = "".join(file_name_part)
    with open(osp.join(base_path, file_name), "w",) as f:
    ...
    if save_path is not None:
        ...
        with open(save_path, "w") as f:
            for key in aucs.keys():
                ...
                f.write(f"{aucs[key]}\n")
                f.write("--------------\n")
    print("scene total aucs:", aucs)
    return aucs


def scene_registration_rate(registrated_state_dict, save_path=None):
    """
    Calculate and save registrated rate
    Parameters:
    --------------
    registrated_state_dict: Dict
        {bag_name: [registrated_mask]}
    """
    overall_registrated_state = list(chain(*registrated_state_dict.values()))
    overall_registration_rate = np.array(overall_registrated_state).sum() / (
        np.array(overall_registrated_state).shape[0] + 1e-6
    )
    registration_analyse = {}
    for key, value in registrated_state_dict.items():
        sub_bag_analyse = {}
        sub_bag_analyse["num image in bag"] = len(value)
        sub_bag_analyse["num image registred"] = np.array(value).sum()
        sub_bag_analyse["registration rate of bag"] = np.array(value).sum() / (
            np.array(value).shape[0] + 1e-6
        )
        registration_analyse[key] = sub_bag_analyse

    print("overall reigistration rate", overall_registration_rate)
    if save_path is not None:
        assert osp.exists(save_path.rsplit("/", 1)[0])
        with open(save_path, "w") as f:
            f.write(f"Over all registration rate : {overall_registration_rate}\n")
            f.write("-----------------------\n")
            for bag_name, analyses in registration_analyse.items():
                f.write(bag_name + ":" + "      ")
                for metrics_name, metrics_value in analyses.items():
                    f.write(metrics_name + ":" + f"{metrics_value}" + "      ")
                f.write("\n")


@ray.remote(num_cpus=1, num_gpus=1, max_calls=1)  # release gpu after finishing
def matcher_colmap_eval(subset_bag, args, pba: ActorHandle):
    error_dict = {}
    registration_mask_dict = {}
    args_reserve = copy.deepcopy(args)
    for subset_name in subset_bag:
        logger.info(f"Optimize bag: {subset_name}")
        args = copy.deepcopy(args_reserve)
        args.save_dir = osp.join(args.save_dir, osp.splitext(subset_name)[0])
        args.cache_dir = osp.join(args.save_dir, "raw_matches")
        ...
        if args.visual_enable:
            ...
            bag_registred_mask = visualize_colmap(dataset, keypoints_dict, args)
        else:
            raise NotImplementedError

        # Eval colmap results:
        logger.info("Eval colmap results running!")
        bag_err_dict = eval_colmap_results(dataset, args)

        error_dict[osp.splitext(subset_name)[0]] = bag_err_dict
        registration_mask_dict[osp.splitext(subset_name)[0]] = bag_registred_mask

        pba.update.remote(1)
    return error_dict, registration_mask_dict
...
def main():
    ...
    subset_list = sorted(os.listdir(subset_path))

    random.shuffle(subset_list)

    # subset_list = subset_list[:3]  # debug only
    # subset_list.insert(0, "25bag_000.txt")

    pb = ProgressBar(len(subset_list), "Run matcher, colmap, and results evaulator")
    ...
    obj_refs = [
        matcher_colmap_eval.remote(subsets, copyed_args[i], pb.actor)
        for i, subsets in enumerate(all_subsets)
    ]
    pb.print_until_done()
    results = ray.get(obj_refs)

    # Aggregation results of a scene
    error_dict = dict(ChainMap(*[k for k, _ in results]))
    registrated_state = dict(ChainMap(*[k for _, k in results]))

    pose_error_name_part = ["scene_error_dict"]
    pose_error_name_part.append(
        "_colmap_err"
    ) if args.use_imc_pose_error_method else pose_error_name_part.append("_loftr_err")
    pose_error_name_part.append("_discard_penalty") if args.discard_penalty else None
    pose_error_name_part.append(".pkl")
    pose_error_name = "".join(pose_error_name_part)
    save_obj(
        error_dict, osp.join(args.save_dir, pose_error_name),
    )
    save_obj(registrated_state, osp.join(args.save_dir, "scene_registrated_state.pkl"))
    ...
    for bag_name, bag_error in error_dict.items():
        if "5bag" in bag_name and "25bag" not in bag_name:
            err_dict_bag5[bag_name] = bag_error
        elif "10bag" in bag_name:
            err_dict_bag10[bag_name] = bag_error
        elif "25bag" in bag_name:
            err_dict_bag25[bag_name] = bag_error
    ...
    scene_pose_auc(
        ...
        save_path=osp.join(
            args.save_dir,
            "scene_pose_auc.txt"
            if not args.discard_penalty
            else "scene_pose_auc_discard_penalty.txt",
        ),
    )
    ...
    scene_pose_auc(
        ...
        save_path=osp.join(
            args.save_dir,
            "scene_pose_auc_bag5.txt"
            if not args.discard_penalty
            else "scene_pose_auc_discard_penalty_bag5.txt",
        ),
    ) if len(err_dict_bag5) != 0 else print("No bag 5!")
    ...
    scene_pose_auc(
        ...
        save_path=osp.join(
            args.save_dir,
            "scene_pose_auc_bag10.txt"
            if not args.discard_penalty
            else "scene_pose_auc_discard_penalty_bag10.txt",
        ),
    ) if len(err_dict_bag10) != 0 else print("No bag 10!")
    ...
    scene_pose_auc(
        ...
        save_path=osp.join(
            args.save_dir,
            "scene_pose_auc_bag25.txt"
            if not args.discard_penalty
            else "scene_pose_auc_discard_penalty_bag25.txt",
        ),
    ) if len(err_dict_bag25) != 0 else print("No bag 25!")
    scene_registration_rate(
        registrated_state,
        save_path=osp.join(args.save_dir, "scene_registration_rate.txt"),
    )
...
def colmapRunner_for_huandi(
    keypoints_dict,
    matches_dict,
    image_dir,
    colmap_temp_dir,
    colmap_output_dir,
    colmap_verbose=False,
    debug=False,
    colmap_filter_max_rproj_err=10,
):
    r""" Run colmap use own matches.

    Args:
        keypoints_dict (``dict``): {filename: keypoints (``np.ndarray``) N*2}.
        matches_dict (``dict``): {filename0-filename1: matches_indexs (``np.ndarray``) 2*M}
        img_dir (``str``): image directory
        colmap_temp_dir (``str``): store intermedite resuts
        colmap_output_dir (``str``): store colmap results
    """
    # create temp directory
    if osp.exists(colmap_temp_dir):
        logger.info(" -- temp path exists - cleaning up from crash")
        rmtree(colmap_temp_dir)
        if os.path.exists(colmap_output_dir):
            rmtree(colmap_output_dir)

    # create output directory
    if osp.exists(colmap_output_dir):
        if not debug:
            logger.info("colmap results already exists, don't need to run colmap")
            return
        else:
    ...
    os.makedirs(colmap_temp_dir)
    os.makedirs(colmap_output_dir)

    # Create colmap-friendy structures
    os.makedirs(os.path.join(colmap_temp_dir, "images"))
    os.makedirs(os.path.join(colmap_temp_dir, "features"))
    assert osp.exists(image_dir)
    img_names = sorted(os.listdir(image_dir), reverse=False)
    img_paths = [osp.join(image_dir, img_name) for img_name in img_names]
    pair_ids = list(combinations(range(len(img_names)), 2))

    # TODO: add use visible

    # copy images
    for _src in img_paths:
        _dst = osp.join(colmap_temp_dir, "images", osp.basename(_src))
        copyfile(_src, _dst)
    logger.info(f"Image copy finish! Copy {len(img_paths)} images!")
    ...
    for img_path in img_paths:
        ...
        keypoints = keypoints_dict[f_name]
        # kpts file to write to:
        kp_file = osp.join(colmap_temp_dir, "features", img_name + ".txt")
        num_kpts.append(keypoints.shape[0])
        # open file to write
        with open(kp_file, "w") as f:
            # Retieve the number of keypoints
    ...
    # write matches to colmap friendly format
    match_file = os.path.join(colmap_temp_dir, "matches.txt")
    num_matches = []
    with open(match_file, "w") as f:
        for i, pair_id in enumerate(pair_ids):
            img0_name = os.path.basename(img_paths[pair_id[0]])
    ...
    try:
        print("COLMAP Feature Import")
        cmd = ["colmap", "feature_importer"]
        cmd += ["--database_path", os.path.join(colmap_output_dir, "databases.db")]
        cmd += ["--image_path", os.path.join(colmap_temp_dir, "images")]
        cmd += ["--import_path", os.path.join(colmap_temp_dir, "features")]
        colmap_res = subprocess.run(cmd)
        ...
        cmd = ["colmap", "matches_importer"]
        cmd += ["--database_path", os.path.join(colmap_output_dir, "databases.db")]
        cmd += ["--match_list_path", os.path.join(colmap_temp_dir, "matches.txt")]
        cmd += ["--match_type", "raw"]
        ...
        cmd += ["--image_path", os.path.join(colmap_temp_dir, "images")]
        cmd += ["--database_path", os.path.join(colmap_output_dir, "databases.db")]
        cmd += ["--output_path", colmap_output_dir]
        # cmd += ["--Mapper.min_model_size", str(colmap_min_model_size)]
        ...
        cmd += [
            "--Mapper.filter_max_reproj_error",
            str(colmap_filter_max_rproj_err),
        ]
        ...
        if colmap_verbose:
            ...
            colmap_res = subprocess.run(cmd, capture_output=True)
            with open(osp.join(colmap_output_dir, "output.txt"), "w") as f:
                f.write(colmap_res.stdout.decode())
