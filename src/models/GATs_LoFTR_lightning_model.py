...
class PL_GATsLoFTR(pl.LightningModule):
    def __init__(self, *args, **kwargs):
        ...
        self.n_vals_plot = max(self.hparams['trainer']['n_val_pairs_to_plot'] // self.hparams['trainer']['world_size'], 1)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {'loss_scalars': batch['loss_scalars'], 'figures': figures, 'metrics': metrics}
    ...
    def validation_epoch_end(self, outputs):
        ...
        for valset_idx, outputs in enumerate(multi_outputs):
            ...
            loss_scalars = {k: flattenList(gather([_ls[k] for _ls in _loss_scalars])) for k in _loss_scalars[0]}
            ...
            if self.trainer.global_rank == 0:
                for k, v in loss_scalars.items():
                    ...
                    self.logger.experiment[0].add_scalar(f'val_{valset_idx}/avg_{k}', mean_v, global_step=cur_epoch)

                val_metrics_4tb = aggregate_metrics(metrics, self.hparams['eval_metrics']["pose_thresholds"])
                for k, v in val_metrics_4tb.items():
                    self.logger.experiment[0].add_scalar(f"metrics_{valset_idx}/{k}", v, global_step=cur_epoch)
