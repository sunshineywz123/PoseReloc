...
# --------------------------------------------------------------------*/
# %BANNER_END%
# from src.datasets.utils_phoaug import darkAug
from src.utils.augmentation.utils_phoaug import darkAug
...
class SuperPointEC(BaseModel):
    ...
    def _forward(self, data, mode='train'):
        ...
        if self.ec_version == 'v1':  # first remove dustbin and then pick the highest position.
            ...
            keypoints = torch.stack([grids[..., 0] + indices % 8, grids[..., 1] + indices // 8], dim=-1)
            if self.align_center_with_resnet:
                keypoints += 4
            ...
            keypoints = [torch.nonzero(s, as_tuple=False).flip(1) for s in scores]  # [N, (n_kpts, 2)]
