...
class LoFTRLoss(nn.Module):
    ...
    def compute_coarse_loss(self, conf, conf_gt, weight=None):
        ...
        if self.config['coarse_type'] == 'cross_entropy':
            ...
            if self.config['spg_spvs']:
                ...
                loss_pos = - alpha * torch.pow(1 - conf[conf_gt == 1], gamma) * (conf[conf_gt == 1]).log()
                loss_neg = - alpha * torch.pow(conf[conf_gt == 0], gamma) * (1 - conf[conf_gt == 0]).log()
                if weight is not None:
                    loss_pos = loss_pos * weight[conf_gt == 1]
                    loss_neg = loss_neg * weight[conf_gt == 0]
                return self.c_pos_w * loss_pos.mean() + self.c_neg_w *loss_neg.mean()
    ...
    def compute_coarse_prior_loss(self, prior0, prior1, conf_gt, weight0=None, weight1=None):
        """
        Args:
        ...
        prior, prior_gt = map(lambda x: torch.cat(x, 0), [[prior0, prior1], [prior0_gt, prior1_gt]])
        alpha, gamma = self.config['coarse_prior']['focal_alpha'], self.config['coarse_prior']['focal_gamma']
        
        loss_pos = - alpha * torch.pow(1 - prior[prior_gt], gamma) * (prior[prior_gt]).log()
        loss_neg = - alpha * torch.pow(prior[~prior_gt], gamma) * (1 - prior[~prior_gt]).log()
    ...
    def compute_fine_rejection_loss(self, conf_f, expec_f_gt):
        ...
        Args:
            ...
            expec_f_gt (torch.Tensor): [M, 2] <x, y>
        """
        alpha, gamma = self.config['fine_rejection']['focal_alpha'], self.config['fine_rejection']['focal_gamma']
        # conf_f = torch.clamp(conf_f, 1e-6, 1-1e-6)
        if expec_f_gt.shape[0] == 0:
            logger.warning("No input for fine-level rejection, might cause ddp deadlock.")
            return None

        pos_mask = torch.linalg.norm(expec_f_gt, ord=float('inf'), dim=1) < 1
        loss_pos = - alpha * torch.pow(1 - conf_f[pos_mask], gamma) * (conf_f[pos_mask]).log()
        loss_neg = - alpha * torch.pow(conf_f[~pos_mask], gamma) * (1 - conf_f[~pos_mask]).log()
        loss = loss_pos.mean() + loss_neg.mean()
    ...
    def forward(self, data):
        ...
        # 1. coarse-level loss
        loss_c = self.compute_coarse_loss(data['conf_matrix'] if not self.config['spg_spvs'] else data['conf_matrix_with_bin'],
                                          data['conf_matrix_gt'], weight=c_weight)
