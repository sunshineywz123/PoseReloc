...
class FinePreprocess(nn.Module):
    ...
    def forward(self, data, feat_3D, feat_2D_db, feat_query_f, feat_3D_c=None, feat_query_c=None,
                feats0=None, feats1=None):
        data.update({'W': self.W})
        self.n_leaf = int(feat_2D_db.shape[-1] / feat_3D.shape[-1])
        if data['b_ids'].shape[0] == 0:
            feat_3D = torch.empty(0, self.config['d_model'], 1, device=feat_3D.device)
            feat_2D_db = torch.empty(0, self.config['d_model'], 1* self.n_leaf, device=feat_2D_db.device)
            feat_query_f = torch.empty(0, getattr(self, 'W_MAX', self.W)**2, self.config['d_model'], device=feat_2D_db.device)
            return feat_3D, feat_2D_db, feat_query_f
        
        if self.feat_ids is None or (len(self.feat_ids) == 2 and not self.ms_feat):
            return self._forward_single_scale(data, feat_3D, feat_2D_db, feat_query_f, feat_3D_c, feat_query_c)
        else:
            raise NotImplementedError
    ...
    def _forward_single_scale(self, data, feat_3D, feat_2D_db, feat_query_f, feat_3D_c=None, feat_query_c=None):
        W = self.W
