...
def parse_args():
    ...
    # Output:
    parser.add_argument(
        "--output_data_dir", type=str, default="/nas/users/hexingyi/yolo_data"
    )
        ...
        for i, obj_pose in enumerate(pose_info):
            if obj_id == str(obj_pose["obj_id"]):
                ...
                cad_model_path = osp.join(
                    model_dir,
                    "obj_" + "0" * (6 - len(obj_id)) + obj_id + ".ply",
                )
...
if __name__ == "__main__":
    ...
    if args.dataset_name != "lmo":
        output_data_obj_dir = osp.join(
            ...
            "-".join(
                [
                    args.assign_onepose_id,
                    args.dataset_name + str(int(args.obj_id)),
                    "others",
                ]
            ),
        )
        sequence_name = args.split
    else:
    ...
    output_image_dir = osp.join(output_data_obj_dir, "images", sequence_name)
    output_label_dir = osp.join(output_data_obj_dir, "labels", sequence_name)
    ...
    for global_id, (image_path, image_info) in tqdm(
        ...
        img_base_name, img_ext = osp.splitext(img_name)
        ...
        # Save to aim dir:
        if args.split == "val":
            os.symlink(image_path, osp.join(output_image_dir, img_name))
            # np.savetxt(osp.join(output_label_dir, img_base_name + ".txt"), [0, x_center, y_center, w_norm, h_norm])
            with open(osp.join(output_label_dir, img_base_name + ".txt"), "w") as f:
                f.write(" ".join([str(0), str(x_center), str(y_center), str(w_norm), str(h_norm)]))
        elif args.split == "train":
            os.symlink(image_path, osp.join(output_image_dir, str(global_id) + img_ext))
            # np.savetxt(osp.join(output_label_dir, str(global_id) + ".txt"), [0, x_center, y_center, w_norm, h_norm])
