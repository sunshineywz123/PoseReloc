...
def parse_args():
    ...
    parser.add_argument("--split", type=str, default="train", choices=["train", "val_rendered"])
...
def parse_data_for_obj(
    ...
    assert osp.exists(img_dir) and osp.exists(model_dir)
    ...
    for global_id, (img_id, pose_info) in enumerate(gt_pose_dic):
        # if global_id > 100:
        #     break
        assert len(pose_info) == 1
        obj_index = str(0)  # index of the obj in image, useful for parse semantic mask
        R_m2c = np.array(pose_info[0]["cam_R_m2c"]).reshape(3, 3)  # 3*3
        t_m2c = np.array(pose_info[0]["cam_t_m2c"])[:, None]  # 3*1
...
if __name__ == "__main__":
    ...
    sequence_name = "-".join(
        [args.dataset_name + args.obj_id, "1" if args.split == "train" else "3"]
    ...
    for global_id, (image_path, image_info) in tqdm(
        ...
        if global_id == 0 and args.split == "train":
            ...
            np.savetxt(
                osp.join(output_data_obj_dir, "diameter.txt"), np.array([diameter])
            )
