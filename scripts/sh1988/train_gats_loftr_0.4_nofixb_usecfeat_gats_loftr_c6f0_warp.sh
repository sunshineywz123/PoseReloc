...
srun --partition=3d_share --nodes=$n_nodes --gres=gpu:$n_gpus_per_node --ntasks-per-node=8 --mem=0 --job-name=$job_name python -u ./train_gats_loftr.py +experiment=train_GATs_loftr_1988_fine_w5_7000_tmp_0.08_use_coarse_nofb_no_fineatt_pe_linear_gats_loftr_c6f0_warp trainer.gpus=$n_gpus_per_node trainer.num_nodes=$n_nodes task_name=$task_name